...
def preprocess_audio(file_path):
    try:
        ...
        n_mels = 16  # Further reduce n_mels
...
# Training loop
def train_diffusion_model(dataset, epochs=200, noise_level=0.02):
    model = UNet()
...
def denoise_audio(model, file_path, output_folder="denoised_outputs"):
    ...
    n_fft = 1024
    n_stft = n_fft // 2 + 1
    inv_transform = torchaudio.transforms.InverseMelScale(n_mels=16, n_stft=n_stft)

    try:
        denoised_waveform = inv_transform(denoised_spectrogram)

        if denoised_waveform.abs().max() > 0:
            denoised_waveform /= denoised_waveform.abs().max()
            denoised_waveform = denoised_waveform.unsqueeze(0)  # Add channel dimension
            waveform_int16 = (denoised_waveform * 32767).to(torch.int16)
...
if len(dataset) == 0:
    ...
    for file in dataset:
        ...
        denoised_audio, sr = denoise_audio(trained_model, file,output_folder="denoised_outputs")
