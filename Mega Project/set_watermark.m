...
[x,fs] = audioread('high freq audio input.mp3');
x = x(1:30*fs); x = x(:);
L = length(x);
N = 512;
win = blackman(N+1);
...
im = imread('input img1.jpg');
[h, w] = getImageDimensions(im);
fprintf('Image dimensions: %d x %d pixels with %d channels\n', h, w);
S = size(im);
row = 1;
column = 1;
...
y = y/max(abs(y));
y=real(y);
audiowrite('output audio trim.wav',y,fs);